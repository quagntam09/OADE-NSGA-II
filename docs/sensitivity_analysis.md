...
Phan tich nay tach rieng khoi benchmark chinh. No khong dung den:

- `config/benchmark.yaml`
- `src/plotting/all_problems.py`

## Noi dung kiem dinh

Lenh `sensitivity` trong `main.py` chay rieng OADE-NSGA-II de kiem tra:

- `pop_size`: `50`, `100`, `200`
- `stagnation_patience`: `5`, `10`, `15`, `20`, `25`, `30`, `40`
- `f_cr_init`: fixed baseline, low init, high init, random uniform

Mac dinh moi cau hinh chay `30` lan voi seed `42..71`, tren cac bai toan ZDT trong `config/sensitivity.yaml`.
...
```powershell
.\.venv\Scripts\python.exe main.py sensitivity
```

Ve bieu do sau khi co ket qua:

```powershell
.\.venv\Scripts\python.exe main.py plot-sensitivity
```

Logic ve bieu do nam trong `src/plotting/parameter_sensitivity.py`.

## Output

Tat ca ket qua nam trong `results/sensitivity/`:
...
- Neu muon chay nhanh de thu pipeline, sua `runs` hoac `n_gen` trong `config/sensitivity.yaml`.
